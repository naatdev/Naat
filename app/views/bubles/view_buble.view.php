...
<div class="row">
    ...
    <div class="col col-padding col-lg-10">
        <p class="small_indic">Retrouvez ici les dernières actualités des personnes que vous avez ajoutées à cette bulle</p>
    </div>
</div>
<div class="clear"></div>

<?php
$results = $db->query('SELECT "USERNAME" FROM listing_bubles_who WHERE "BUBLE_ID" = "'.$id.'" ORDER BY "ADD_TIME" DESC');
$usernames = array();
        ...
        while($row_ = $results_->fetchArray()) {
            $nbr++;
                ...
                if($row_['TYPE'] == "NEW_PROFILE_PIC") {
                    ...
                    if($row_['NO_FORMAT'] != 1) {
                        $post['post_content'] = $parsedown->setBreaksEnabled(true)->text($emojis->transformToSvg($row_['CONTENT']));
                    }
                }
